...
echo "<form method='post' action='".$url_prog_en."' name='main'>";
...
if (mysql_num_rows($dayres)) {
  ...
  while ($the_day=mysql_fetch_array($dayres)) {
    echo "<option value='".$the_day['slot_date']."'>".date('F, d (l)',strtotime($the_day['slot_date']))."</option>";
  }
  echo "</select>";
}
echo '<input type="submit" name="submitBtn" value="Select" onClick="" />';
echo "</form>";
...
if ($sel_day!='') {
  ...
  echo "<center><h2>".date('F, d (l)',strtotime($sel_day))."</h2></center>";
...
while ($the_slot=mysql_fetch_array($slotres)) {
  ...
  while ($the_session=mysql_fetch_array($sessionres)) {
    ...
    echo "Chair: <em>".utf8_encode($the_session['chair'])."</em></center>";
    ...
    while ($the_paper=mysql_fetch_array($papersres)) {
      ...
      if (time() >= strtotime($deadline_papers)) {
        if ($the_paper['isUploaded']=='Y') {
          ...
          echo "<a href='".$filepath."' target='_blank'>Long abstract (PDF)</a> - ";
         }
        $abstractpath = $url_showabstract."?id=".$paperid;
        echo "<a href='".$abstractpath."' target='_blank'>Short abstract</a><br />";
      }
      $curnb=$curnb-1;
      if ($curnb==0) {
        echo "</font>";
